...
def run_atlas(synthese_queries, output_dirs, area_name, run_render=True):
    ...
    # 1. Données
    with timer.step("Requetes get_atlas_species_grid"):
        grid_rows = synthese_queries.get_atlas_species_grid()
    with timer.step("Requetes get_atlas_species_summary"):
        summary_rows = synthese_queries.get_atlas_species_summary()
    with timer.step("Requetes get_area_zone"):
    ...
    with timer.step("Copie du projet QGIS Atlas"):
        template_path = (
            Path(__file__).resolve().parents[2] / "templates" / "atlas_modele.qgs"
        )
    ...
    # 4. Rendu atlas QGIS
    with timer.step("Rendu atlas QGIS"):
